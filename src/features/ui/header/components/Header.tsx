...
export default function Header({showSidebar,setShowSidebar,dispatch,state}:HeaderProps) {
    ...
    const [loginSession, setLoginSession]=useState<loginProps>(loginInitialState)
...
console.log(productsSend?.Products)
    useEffect(()=>{
        if(!state)return
        ...
        const data = {
            ...
            'Products':state.cart.map(products => ({
                ...
                'Amount_product':products.quantity ? products.quantity.toString() : '1',
        ...
        <header className={HeaderCSS.header}>
            {showFinishBuy && (success ? 
                <div className={HeaderCSS.finishBuy}>
                    ...
                    <div>
                        <h2 className={HeaderCSS.finishBuyTitle}>Confirmar compra</h2>
                        <p>Para confirmar la compra vaya a la sección "Tus Compras" y sube el capture del pago</p>
                    </div>
                </div>
                : 
                <div className={HeaderCSS.finishBuy}>
                    <h2 className={HeaderCSS.finishBuyTitle}>Error</h2>
                    <p>Espere un momento y vuelva a intentar, si el problema persiste contacte al numero telefonico</p>
                </div>
            )}
            ...
            <div className={HeaderCSS.buttonLoginCartContainer}>
                {state?.userLoggedIn ? 
                    <div className={HeaderCSS.containerInfoLogged}>
                        <button title='logut' type='button' className={HeaderCSS.buttonLogout}><img src={LogoutIcon}  alt="" /></button>
                        <p>{loginSession.Name_user + ' ' + loginSession.Last_name}</p>
                    </div>
                    :
                    <button type='button' className={HeaderCSS.buttonLogin} onClick={()=>navigate('user/login/auth')}>
                        Login
                        <img src={LoginIcon} alt="Entrar" />
                    </button>
                }
                <button title='cart' type='button' onClick={()=>{
                ...
                </button>
                {
                    showCart && (
                        ...
                        <div className={HeaderCSS.contentCart}>
                            ...
                            {state?.userLoggedIn ?
                                <ButtonRegisterLogin title={'Comprar'} disabled={state?.cart === undefined} onClick={()=>{
                                if(!state)return
                                dispatch({type:'sendBuyCart',payload:{buy:productsSend}})
                                setTimeout(()=>{
                                ...
                                : 
                                <h3 className={HeaderCSS.textCartNoLogged}>Debe estar logeado para comprar</h3>
                            }
