...
import { loginProps, LoginUser } from "../login/helpers/LoginUser";
import {
	CartItem,
	SendBuy,
...
export type registerActions = {
	type: "registerUSer";
	payload: { user: RegisterUserInterface };
};
...
export const initialState = {
	...
	userLoggedIn: sessionStorageLogin
		? JSON.parse(sessionStorageLogin)
		: "",
...
export const RegisterReducer = (
	...
	switch (actions.type) {
		case "registerUSer": {
			CreateUser(actions.payload.user)
				.then((response) => {
		...
		case "registerCategory": {
			CreateCategory(actions.payload.category.name)
				.then((response) => {
					console.log(response);
		...
		case "registerProduct": {
			CreateProduct(actions.payload.product)
				.then((response) => {
					console.log(response);
					setMsj(response.statusText);
					setSuccess(true);
		...
		case "uploadProduct": {
			UploadImageProduct(actions.payload.uploadImages)
				.then((response) => {
					console.log(response);
		...
		case "addToCart": {
			const productExist = state.cart.find(
				(product) => product.id === actions.payload.cart.id
			);
			...
			if (
				productExist
					...
					if (product.id === actions.payload.cart.id && product.quantity < product.quantityMax) {
						return {
							...product,
		...
		case "increaseQuantity": {
			const cart = state.cart.map((product) => {
				if (
					product.id === actions.payload.id && product.quantity < product.quantityMax
				) {
					return {
						...product,
						quantity: product.quantity + 1,
